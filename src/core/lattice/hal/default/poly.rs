...
pub trait PolyInterface<T>: Clone + Debug + PartialEq + From<Vec<T>> {
    ...
    fn GetFormat(&self) -> Format;
    fn SetFormat(&mut self, format: Format);
    fn GetParams(&self) -> &Params;
    fn GetValues(&self) -> &Vec<T>;
    fn SetValues(&mut self, values: Vec<T>, format: Format);
    fn SetValuesToZero(&mut self);
    fn SetValuesToMax(&mut self);
    fn Plus(&self, rhs: &Self) -> Self;
    fn PlusNoCheck(&self, rhs: &Self) -> Self;
    fn Minus(&self, rhs: &Self) -> Self;
    fn Times(&self, rhs: &Self) -> Self;
    fn TimesNoCheck(&self, rhs: &Self) -> Self;
    fn MultiplyAndRound(&self, p: &T, q: &T) -> Self;
    fn DivideAndRound(&self, q: &T) -> Self;
    fn Negate(&self) -> Self;
    fn AddILElementOne(&mut self);
    fn AutomorphismTransform(&self, k: u32) -> Self;
    fn AutomorphismTransformWithVec(&self, k: u32, vec: &Vec<u32>) -> Self;
    fn MultiplicativeInverse(&self) -> Self;
    fn ModByTwo(&self) -> Self;
    fn Mod(&self, modulus: &T) -> Self;
    fn SwitchModulus(&mut self, modulus: &T, root_of_unity: &T, modulus_arb: &T, root_of_unity_arb: &T);
    fn SwitchFormat(&mut self);
